...
function createPDF(text, width, height, columns) {
    ...
    var lines = wrapText(text, maxWidth, ctx);
...
function generatePDFBlob(pdf) {
    ...
    doc.setFontSize(pdf.pdfData.fontSize * 0.75);
    
    var padding = pdf.width * 0.1;
    var startY = (pdf.height - pdf.pdfData.textLines.length * pdf.pdfData.fontSize * 0.75 * 0.0353 * pdf.pdfData.lineHeight) / 2;
    
    for (var i = 0; i < pdf.pdfData.textLines.length; i++) {
        var yPos = startY + i * pdf.pdfData.fontSize * 0.75 * 0.0353 * pdf.pdfData.lineHeight;
