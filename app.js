...
function hyphenateText(text, maxWidth, ctx, fontSize) {
...
function createPDF(text, width, height) {
    ...
    for (var size = 12; size < 500; size += 2) {
        ...
        lines = hyphenateText(text, maxWidth, ctx, size);
        var totalHeight = lines.length * size * lineHeight;
        
        if (totalHeight > maxHeight) {
            fontSize = size - 2;
            ctx.font = 'bold ' + fontSize + 'px Arial, sans-serif';
            lines = hyphenateText(text, maxWidth, ctx, fontSize);
            break;
        }
        fontSize = size;
    }
    ...
    var startY = (canvas.height - totalTextHeight) / 2 + fontSize / 2;
